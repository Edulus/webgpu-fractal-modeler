...
struct Uniforms {
  ...
  detail       : vec4<f32>,
  // Mathematical parameters of the active shape, eight floats as two vec4.
  // Only the shape being drawn has values here; every other slot is zero. Each
  // estimator documents which slot it reads. Dedicated storage, NOT packed into
  // another control's bits -- an earlier attempt smuggled these through
  // imageAdjust and the composite pass had to decode its own controls back out.
  shapeParams  : array<vec4<f32>, 2>,
};

// Slot accessor, so estimators read shapeParam(1u) rather than indexing into
// a vec4 array by hand and getting the component arithmetic wrong.
fn shapeParam(i : u32) -> f32 {
  let v = u.shapeParams[i / 4u];
  let k = i % 4u;
  if (k == 0u) { return v.x; }
  if (k == 1u) { return v.y; }
  if (k == 2u) { return v.z; }
...
// Menger recursion ceiling. Its live depth is a SHAPE parameter rather than a
// quality one -- deMenger never read u.detail.y, so nothing is taken from the
// governor by exposing it.
const MENGER_MAX : i32 = 8;
...
fn deMandelbulb(pos : vec3<f32>) -> DEResult {
  ...
  var r = 0.0;
  var trap = 1e10;
  // Slot 0 power, slot 1 bailout. deIters stays the governor's: mathematical
  // parameters and the rendering budget are deliberately separate controls.
...
fn deMandelbox(pos : vec3<f32>) -> DEResult {
  // Slots 0..2: fold scale, inner radius, outer radius.
  let scale = shapeParam(0u);
  let fixedR = shapeParam(2u);
  // The inner radius is held strictly below the outer one HERE as well as in
  // the registry. Overshoot was measured at the exact point the two cross, and
  // the estimator must never evaluate an inverted pair however the uniform came
  // to hold one -- a clamp in JS alone would leave the shader trusting input.
  let minR = min(shapeParam(1u), fixedR - 0.01);
  let minR2 = minR * minR;
  let fixedR2 = fixedR * fixedR;
  var z = pos;
  var dr = 1.0;
  var trap = 1e10;
  let deIters = i32(u.detail.y);
...
fn deMenger(pos : vec3<f32>) -> DEResult {
  ...
  var s = 1.0;
  var colorTrap = 0.0;
  // Slot 0: mathematical recursion depth. The ceiling is a compile-time
  // constant and the live value breaks out early, the same shape the governor's
  // iteration budget uses -- but this one is the user's, not the governor's.
  let depth = i32(shapeParam(0u));
  for (var m = 0; m < MENGER_MAX; m = m + 1) {
    if (m >= depth) { break; }
...
fn deJulia(pos : vec3<f32>) -> DEResult {
  // Slots 0..3: the quaternion constant itself. This used to orbit on a timer;
  // a value cannot be both animated and controlled, so the sliders own it and
  // their defaults are the pose that timer held at t = 0.
  let c = vec4<f32>(
    shapeParam(0u), shapeParam(1u), shapeParam(2u), shapeParam(3u)
  );
