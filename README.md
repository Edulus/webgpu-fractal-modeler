...
```text
├── index.html                    interactive shape-explorer and background demo
├── src/
...
- It always plays at least `MIN_STEPS` steps, set to the **whole sequence**. Walking the panel is the point of the tour, and a floor shorter than the sequence would make which controls you are taught depend on how fast your GPU is. A floor is needed at all because without one a fast start shows a single highlight for 200ms and rips it away — a flash the eye is drawn to and then cannot find.

In practice the floor is what decides the length and readiness never enters into it: seven steps at 2.2s is 15.4s of tour against a ~640ms start, so the card is gone at ~800ms and the tour runs on with the panel live underneath it. Only a start slower than the whole tour reaches the rounding rule, and then the last step simply holds until the renderer is up. `MIN_STEPS` is the one number to change to shorten it — at 2 it plays Shape and Palette and stops, roughly the length of a quick start. Touching any control, pressing Escape, or clicking Skip ends it immediately — being talked at about Shape while already changing Quality is the most annoying thing an onboarding tour can do.
...
A cube lattice terraced by Chebyshev distance:

    h(i, j) = STEP * max(|i|, |j|)

The choice of norm is the entire design. The L∞ distance has *square* level sets, so the terraces are concentric square rings; L² would give circular terraces and L¹ diamonds. Seen from near grazing, those square rings compress into nested chevrons converging on the vanishing point, which is why the result reads as far more elaborate than the one line of arithmetic that generates it — the complexity is perspective, not the rule.
...
**It works to a budget, not a race — and reads that budget as a miss rate.** `requestAnimationFrame` is vsync-locked, so observed frame time is *quantised*: at 60Hz a frame is reported as 16.7ms or 33.3ms and never anything between. A rule based on mean frame time therefore cannot see headroom at all. The first version of this governor used one, and a simulated workstation doing **1.0ms of work per frame reported 16.7ms like everything else and never climbed off its starting rung** — no display can report the 12ms that rule wanted. (The FPS thresholds it replaced survived this by luck: 1000/16.7 = 59.9, just over their 58.) What a vsync-locked display *can* report is whether a frame landed on the next refresh or the one after, so the governor counts the fraction of frames that missed the budget. That reads correctly whether timing is quantised or continuous.

**A rung that misbehaves is remembered, and re-probed ever more rarely.** On a drop the governor settles one rung *below* the level that stalled and records it. The ceiling lifts again after a quiet stretch — a view that has become cheaper should not be punished forever — but each repeat failure at the same rung doubles the wait, up to five minutes. That is what keeps a **thermally throttled** device settled: without the backoff a simulated laptop heating up re-probed the same stall 60 times; with it, about once every hundred seconds over twenty simulated minutes. A single frame far over budget is acted on at once rather than waiting out the hysteresis.

**Converged frames are still excluded**, though the miss-rate signal turns out to be sturdier here than the mean was: fed a 50/50 mixture of converged and interactive frames it still finds the overload. It breaks at the ratio that actually occurs — while the accumulator runs, most frames are converged — and at nine-in-ten the on-time frames hold the miss rate under the threshold while every interactive frame is late. The test covers both ratios so the boundary is on record.
...
The governor is pure — no WebGPU, no DOM, no clock of its own — so `tools/quality.test.js` can drive it with synthetic frame times from simulated devices. Every device model presents its frames the way a display does — work rounded up to a whole refresh period — because that is the only signal the real governor will ever get. Against those models it settles at rung 2 on a phone, rung 5 on a laptop and rung 9 on a workstation, in every case inside the frame budget, with two or fewer rung changes in the last third of a 30,000-frame run and fewer than in the first third.
